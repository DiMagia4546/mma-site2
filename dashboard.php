...
$stmtFavFighters = $conn->prepare(
    "SELECT f.* FROM {$favTable} uf
     JOIN fighters f ON uf.fighter_id = f.id
     WHERE uf.user_id = ? AND uf.fighter_id IS NOT NULL"
);
$stmtFavFighters->bind_param("i", $user_id);
$stmtFavFighters->execute();
$fav_fighters = $stmtFavFighters->get_result();

$stmtFavEvents = $conn->prepare(
    "SELECT e.* FROM {$favTable} uf
     JOIN events e ON uf.event_id = e.id
     WHERE uf.user_id = ? AND uf.event_id IS NOT NULL"
);
$stmtFavEvents->bind_param("i", $user_id);
$stmtFavEvents->execute();
$fav_events = $stmtFavEvents->get_result();

$stmtActivity = $conn->prepare(
    "SELECT * FROM user_activity WHERE user_id = ? ORDER BY created_at DESC LIMIT 10"
);
$stmtActivity->bind_param("i", $user_id);
$stmtActivity->execute();
$activity = $stmtActivity->get_result();
?>
<!DOCTYPE html>
<html lang="pt">
<head>
...
<section class="max-w-6xl mx-auto px-6">
    ...
    <div class="bg-neutral-800 border border-neutral-700 rounded-xl p-8 shadow-xl mb-12">
        ...
        <div class="flex items-center gap-6 mb-10">
            <img src="<?= e($user['profile_pic'] ?: 'uploads/default_user.png') ?>" class="w-28 h-28 rounded-full object-cover border-2 border-red-600" alt="Foto de perfil">

            <div>
                <h2 class="text-4xl font-bold"><?= e($user['name']) ?></h2>
                <p class="text-neutral-400"><?= e($user['email']) ?></p>

                <?php if ($user['role'] === 'admin'): ?>
                    <span class="inline-block mt-2 bg-red-600 text-white px-3 py-1 rounded-full text-sm font-bold">ADMINISTRADOR</span>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-neutral-900 border border-neutral-700 rounded-xl p-6">
                ...
                <p class="text-2xl font-bold"><?= e($user['name']) ?></p>
            </div>

            <div class="bg-neutral-900 border border-neutral-700 rounded-xl p-6">
                <p class="text-neutral-400 text-sm">Email</p>
                <p class="text-2xl font-bold"><?= e($user['email']) ?></p>
            </div>

            <div class="bg-neutral-900 border border-neutral-700 rounded-xl p-6">
                <p class="text-neutral-400 text-sm">Conta criada em</p>
                <p class="text-xl font-bold"><?= e(date("d/m/Y", strtotime($user['created_at']))) ?></p>
            </div>
        </div>

        <div class="mt-10 flex gap-6">
            <a href="editar_perfil.php" class="bg-red-600 px-6 py-3 rounded-lg hover:bg-red-700 transition">Editar Perfil</a>
            <a href="alterar_password.php" class="bg-neutral-700 px-6 py-3 rounded-lg hover:bg-neutral-600 transition">Alterar Password</a>

            <?php if ($user['role'] === 'admin'): ?>
                <a href="admin_panel.php" class="bg-yellow-600 px-6 py-3 rounded-lg hover:bg-yellow-700 transition">Painel Admin</a>
            <?php endif; ?>
        </div>
    </div>
        ...
        <?php if ($fav_fighters->num_rows > 0): ?>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <?php while ($f = $fav_fighters->fetch_assoc()): ?>
                    <a href="fighter.php?id=<?= (int) $f['id'] ?>" class="bg-neutral-900 border border-neutral-700 rounded-xl p-4 hover:bg-neutral-700 transition">
                        <img src="<?= e($f['image']) ?>" class="w-full h-40 object-cover rounded-xl mb-3" alt="<?= e($f['name']) ?>">
                        <p class="text-xl font-bold"><?= e($f['name']) ?></p>
                        <p class="text-neutral-400"><?= e($f['weight_class']) ?></p>
                    </a>
                <?php endwhile; ?>
            </div>
        <?php else: ?>
        ...
        <?php if ($fav_events->num_rows > 0): ?>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <?php while ($e = $fav_events->fetch_assoc()): ?>
                    <a href="evento.php?id=<?= (int) $e['id'] ?>" class="bg-neutral-900 border border-neutral-700 rounded-xl p-4 hover:bg-neutral-700 transition">
                        <p class="text-2xl font-bold"><?= e($e['name']) ?></p>
                        <p class="text-neutral-400"><?= e(date("d/m/Y", strtotime($e['date']))) ?></p>
                    </a>
                <?php endwhile; ?>
            </div>
        <?php else: ?>
            ...
            <ul class="space-y-4">
                <?php while ($a = $activity->fetch_assoc()): ?>
                    <li class="bg-neutral-900 border border-neutral-700 rounded-xl p-4">
                        <p class="text-neutral-300"><?= e($a['action']) ?></p>
                        <p class="text-neutral-500 text-sm"><?= e(date("d/m/Y H:i", strtotime($a['created_at']))) ?></p>
                    </li>
                <?php endwhile; ?>
            </ul>
        <?php else: ?>
