...
$sql_history = "SELECT * FROM fighter_history WHERE fighter_id = ? ORDER BY fight_date DESC";
$stmt_h = $conn->prepare($sql_history);
$stmt_h->bind_param("i", $fighter_id);
$stmt_h->execute();
...
<head>
    ...
    <title><?= htmlspecialchars($f['name']) ?> | MMA 360</title>
...
<div class="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
    ...
    <div class="relative">
        <img src="<?= htmlspecialchars($f['image']) ?>" class="w-full h-[450px] object-cover rounded-xl grayscale">
    ...
    <div>
        <p class="text-red-500 text-2xl uppercase tracking-wide"><?= htmlspecialchars($f['weight_class']) ?></p>

        <h1 class="text-6xl font-bold mt-2"><?= htmlspecialchars($f['name']) ?></h1>

        <?php if (!empty($f['nickname'])): ?>
            <p class="text-2xl text-neutral-400 mb-4">"<?= htmlspecialchars($f['nickname']) ?>"</p>
        <?php endif; ?>

        <p class="text-lg text-neutral-300 mb-6"><?= htmlspecialchars($f['nationality']) ?></p>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div class="bg-neutral-800 border border-neutral-700 rounded-xl p-4 shadow">
                <p class="text-xl font-bold"><?= htmlspecialchars($f['height']) ?> m</p>
                <p class="text-neutral-400">Altura</p>
            </div>

            <div class="bg-neutral-800 border border-neutral-700 rounded-xl p-4 shadow">
                <p class="text-xl font-bold"><?= (int)$f['reach'] ?> cm</p>
                ...
                <div class="bg-neutral-800 border border-neutral-700 rounded-xl p-6 flex items-center justify-between shadow">
                    ...
                    <div class="flex items-center gap-6">
                        ...
                        <?php if (!empty($h['opponent_image'])): ?>
                            <img src="<?= htmlspecialchars($h['opponent_image']) ?>" class="w-20 h-20 rounded-full border-2 border-red-600 object-cover">
                        <?php endif; ?>

                        <div>
                            <p class="text-xl font-bold"><?= htmlspecialchars($f['name']) ?> vs <?= htmlspecialchars($h['opponent_name']) ?></p>
                            <p class="text-neutral-400"><?= htmlspecialchars($h['event_name']) ?> — <?= date("d/m/Y", strtotime($h['fight_date'])) ?></p>
                            <p class="text-neutral-500 text-sm"><?= htmlspecialchars($h['method']) ?> • Round <?= (int)$h['round_number'] ?> • <?= htmlspecialchars($h['time']) ?></p>
                        </div>
                    </div>

                    <span class="text-2xl font-bold 
                        <?= $h['result'] == 'Win' ? 'text-green-400' : ($h['result'] == 'Loss' ? 'text-red-400' : 'text-yellow-400') ?>">
                        <?= strtoupper(htmlspecialchars($h['result'])) ?>
                    </span>
                </div>
            <?php endwhile; ?>
        </div>
...
$isChampion = ($f['wins'] > 20 && ((int) $f['wins'] - (int) $f['losses']) > 10);
?>
<!DOCTYPE html>
<html lang="pt">
<head>
...
<div class="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
    ...
    <div class="relative">
        ...
        <?php if ($isChampion): ?>
            <span class="absolute top-4 left-4 bg-red-600 text-white px-4 py-1 rounded-full text-xl font-bold">CAMPEÃO</span>
        <?php endif; ?>
    </div>
